# -*- coding: utf-8 -*-
"""
Created on Mon Feb 14 00:16:08 2022
...
@task(log_stdout=True)
def report_accuracy(predictions: np.ndarray, test_y: pd.DataFrame, name:str ) -> None:
    ...
    print(f"Model accuracy on test set for dataset {name}: {round(accuracy * 100, 2)}")
...
with Flow("data-science") as flow:
    ruta = 'data/processed'
    file_='split_data_output-{}'.format(dt.datetime.today().strftime('%Y-%m-%d'))
    train_test_dict = (
        LocalResult(dir=ruta)
        .read(location=file_)
        .value
    )
    ...
    report_accuracy(predictions, test_y,file_)
